...
<h2 class="hidden">최신글</h2>

<div class="md:grid md:grid-cols-3 my-3 block w-full latest_top_wr">
    <?php
...
<div class="md:grid md:grid-cols-3 my-3 block w-full latest_wr">
<!-- 최신글 시작 { -->
    <?php
    //  최신글
    $sql = " select bo_table
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
		$lt_style = '';
    	if ($i%3 !== 0 ) $lt_style = "mr-2";
    ?>
    <div class="lt_wr md:w-full">
        <?php
        // 이 함수가 바로 최신글을 추출하는 역할을 합니다.
        // 사용방법 : latest(스킨, 게시판아이디, 출력라인, 글자수);
        // 테마의 스킨을 사용하려면 theme/basic 과 같이 지정
